...
async function main() {
  ...
  for (const route of ROUTES) {
    ...
    // Flat files, not <route>/index.html: Cloudflare Pages serves gallery.html at
    // /gallery, whereas a gallery/ directory makes it 308-redirect to /gallery/,
    // which would break the canonical URLs already in the sitemap and in the wild.
    const target =
      route.path === "/"
  ...
  console.log(
    `prerender: ${ROUTES.length} routes and sitemap written to dist/ (lastmod ${lastmod})`
  );
}
